...
export interface SpreadOptions {
  points: { percentile: number; label: string; selected: boolean }[];
  axisLabel: string;
...
/**
 * Every combination of the controls, as one dot each on a percentile axis.
 *
 * dodgeY rather than a row per combination: twelve labelled rows would be
 * taller than the chart it sits above and would invite reading down the list
 * for the biggest number, which is the behaviour this is meant to discourage.
 * A beeswarm shows the shape -- clustered, or spread from end to end -- in a
 * strip, and the labels come on hover for anyone who wants a particular one.
 */
export function renderSpread(opts: SpreadOptions): SVGSVGElement | HTMLElement {
  const { points, theme, width } = opts;
  const chosen = points.filter((d) => d.selected);
  return Plot.plot({
    width,
    height: 108,
    marginTop: 6, marginBottom: 34, marginLeft: 14, marginRight: 14,
    style: { background: "transparent", color: theme.text, fontSize: "11px" },
    x: {
      domain: [0, 100], label: opts.axisLabel, labelAnchor: "center",
      labelArrow: null, ticks: 5, tickSize: 0, tickPadding: 6,
      tickFormat: (d: number) => `${d}`,
    },
    y: { axis: null },
    color: { type: "identity" },
    marks: [
      // The two tails, so "unusual" has a visible place on the axis rather
      // than being a number the reader has to hold in their head.
      Plot.rect([{ x1: 0, x2: 10 }, { x1: 90, x2: 100 }], {
        x1: "x1", x2: "x2", y1: 0, y2: 1, fill: theme.rangeOuter,
      }),
      Plot.ruleX([50], { stroke: theme.axis, strokeDasharray: "3,3" }),
      Plot.dot(points, Plot.dodgeY({
        x: "percentile", r: 4.5, anchor: "bottom", padding: 1.5,
        fill: (d: { selected: boolean }) => (d.selected ? theme.series[0] : theme.history),
        fillOpacity: (d: { selected: boolean }) => (d.selected ? 1 : 0.5),
        stroke: theme.surface, strokeWidth: 1,
        sort: null,
      })),
      // Drawn again on top: dodgeY can place the selected dot behind a
      // neighbour, and the one the reader is looking at has to be findable.
      Plot.dot(chosen, Plot.dodgeY({
        x: "percentile", r: 4.5, anchor: "bottom", padding: 1.5,
        fill: theme.series[0], stroke: theme.surface, strokeWidth: 1.4,
        sort: null,
      })),
      Plot.tip(points, Plot.pointer(Plot.dodgeY({
        x: "percentile", anchor: "bottom", padding: 1.5,
        fill: theme.surface, stroke: theme.axis, textPadding: 8, fontSize: 12,
        title: (d: { percentile: number; label: string; selected: boolean }) =>
